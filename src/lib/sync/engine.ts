  ...
  const { error } = await supabase.from("servicos").upsert(
    {
      ...
      payment_methods: local.payment_methods ?? null,
      valor_a_vista: local.valor_a_vista ?? null,
      valor_parcelado: local.valor_parcelado ?? null,
      qtd_parcelas: local.qtd_parcelas ?? null,
      lat: local.lat ?? null,
      lng: local.lng ?? null,
